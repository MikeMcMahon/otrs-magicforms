# --
# Kernel/Modules/MagicFormsTicketMessage.pm - to handle customer messages
...
package Kernel::Modules::MagicFormsTicketMessage;
...
sub new {
    ...
    # Create the config for the main ticket we're overriding
    $Self->{Config} = $Self->{ConfigObject}->Get("Ticket::Frontend::CustomerTicketMessage");
    $Self->{MFConfig} = $Self->{ConfigObject}->Get("Ticket::Frontend::$Self->{Action}"); 

    # Check if we have the MFQueue param yet
    my $Form = $Self->{ParamObject}->GetParam(Param => 'MFForm');
    $Self->{FormName} = $Form; 
    my $Queue; 

    my $FieldFilter; 
    if ( $Form ) { 
         $FieldFilter = $Self->{MagicFormsObject}->DynamicFieldsForCustomer(FormName => $Form);
    }
    else { 
        # Try to get the queue from the dest param
        if ( $Self->{ParamObject}->GetParam( Param => 'FormName' ) ) { 
            $Form = $Self->{ParamObject}->GetParam( Param => 'FormName' ); 
    ...
    if ( $Form ) { 
        # Check to ensure that the Form name maps to a queue and that the queue is a valid queue...
        $Queue = $Self->{MagicFormsObject}->GetQueue( FormName => $Form ); 

        # Assign the Queue / QueueID
        $Self->{Queue} = $Queue; 
        $Self->{LogObject}->Log(Priority=>'notice', Message=>$Queue); 
        $Self->{QueueID} = $Self->{QueueObject}->QueueLookup( Queue => $Queue );  
    }
...
sub Run {
    ...
    # Check to ensure that a real queue has been supplied
    if ( !$Self->{Queue} ) { 
        ...
        $Self->{LogObject}->Log(Priority=>'notice', Message=>$Self->{Queue}); 
        return $Self->{LayoutObject}->ErrorScreen(
            Message => 'Invalid Queue Specified!',
            Comment => 'Please contact your administrator', 
        );
    }

    if ( $Self->{Queue} && $Self->{QueueID} ) { 
        $Self->_SetDest( Queue => $Self->{Queue}, QueueID => $Self->{QueueID}, FormName => $Self->{FormName} );
    }

    if ( !$Self->{Subaction} ) {
        # no reason to get the queue default with MF since we're just going to set the queue as a param and post it back to ourselves
        my $QueueDefaultID = $Self->{QueueID};
        ...
        for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
            ...
            $DynamicFieldHTML{ $DynamicFieldConfig->{Name} } =
                ...
                Mandatory => 
                    # Using the magic forms to determine if it is mandatory! 
                    $Self->{MagicFormsObject}->IsRequiredForForm(FormName => $Self->{FormName}, DynamicField => $DynamicFieldConfig->{Name}, Default => 1) == 2,
                LayoutObject    => $Self->{LayoutObject},
                ParamObject     => $Self->{ParamObject},
                AJAXUpdate      => 1,
        ...
        my $FormName = $Self->{ParamObject}->GetParam( Param => 'FormName' ) || ''; 
        ...
        if ( !$To ) {
            ...
            # Set the destination so that AJAX updates work
            $Self->_SetDest( Queue => $To, QueueID => $NewQueueID, FormName => $FormName );
        }
        ...
        for my $DynamicFieldConfig ( @{ $Self->{DynamicField} } ) {
            ...
            if ( IsHashRefWithData( $DynamicFieldConfig->{Config}->{PossibleValues} ) ) {
                ...
                my $ACL = $Self->{TicketObject}->TicketAcl(
                    %GetParam,
                    Action         => $Self->{Action},
                    # This is needed so that the MFs refresh on attachment upload/delete will refresh the ACL's for the proper queue 
                    QueueID        => $NewQueueID,
                    TicketID       => $Self->{TicketID},
                    ReturnType     => 'Ticket',
                    ReturnSubType  => 'DynamicField_' . $DynamicFieldConfig->{Name},
            ...
            if ( !$IsUpload && !$GetParam{Expand} ) {
                $Self->{LogObject}->Log(Priority=>'notice', Message=>$To);
                $ValidationResult = $Self->{BackendObject}->EditFieldValueValidate(
                    DynamicFieldConfig   => $DynamicFieldConfig,
                    PossibleValuesFilter => $PossibleValuesFilter,
                    ParamObject          => $Self->{ParamObject},
                    Mandatory =>
                        # using the magic forms to determine if the field is mandatory! 
                        $Self->{MagicFormsObject}->IsRequiredForForm(FormName => $FormName, DynamicField => $DynamicFieldConfig->{Name}, Default => 1 ) == 2,
                );
            ...
            $DynamicFieldHTML{ $DynamicFieldConfig->{Name} } =
                ...
                PossibleValuesFilter => $PossibleValuesFilter,
                Mandatory =>
                    # using magic forms to determine if the field is mandatory! 
                    $Self->{MagicFormsObject}->IsRequiredForForm(FormName => $FormName, DynamicField => $DynamicFieldConfig->{Name}, Default => 1) == 2,
        ...
        # manually set the form submitted 
        my $MagicFormsDynamicFieldConfig = $Self->{DynamicFieldObject}->DynamicFieldGet( 
            Name => 'MagicForms' 
        );

        if ( IsHashRefWithData($MagicFormsDynamicFieldConfig) ) { 
            my $Success = $Self->{BackendObject}->ValueSet(
                DynamicFieldConfig  => $MagicFormsDynamicFieldConfig,
                ObjectID            => $TicketID,
                Value               => $FormName,
                UserID              => $Self->{ConfigObject}->Get('CustomerPanelUserID'),
            );
        ...
        # error handling
            ...
            $Self->{LogObject}->Log(
                Priority => 'error', 
                Message => "There was an error saving the form submitted",
            ); 
        }
...
# Sets the destination based on the passed in parameter! 
sub _SetDest { 
    my ( $Self, %Param ) = @_; 

    if ( $Param{Queue} && $Param{QueueID} && $Param{FormName} ) {
        
        if ( $Self->{Debug} > 0 ) { 
            $Self->{LogObject}->Log(
                Priority => 'notice', 
                Message => "Setting queue destination: $Param{QueueID}||$Param{Queue}"
            ); 
        }
        ...
        $Self->{LayoutObject}->Block(
            Name => 'QueueDest',
            Data => { 
                Dest => "$Param{QueueID}||$Param{Queue}",
...
sub _MaskNew {
    ...
    $Self->{LayoutObject}->Block(
        Name => 'QueueSelected',
        Data => {
            Name => $Self->{ParamObject}->GetParam( Param => 'Dest' ),
        },
    );
    ...
    return $Self->{LayoutObject}->Output(
        TemplateFile => 'MagicFormsTicketMessage',
        Data         => \%Param,
    );
}
